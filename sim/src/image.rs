...
fn required_slot_padding(dev: &dyn Flash, trailer_size: usize) -> usize {
    ...
    if Caps::SwapUsingMove.present() || Caps::SwapUsingOffset.present() {
        ...
        // For swap-move, ensure enough space is available for the fallback trailer
        if Caps::SwapUsingMove.present() {
           let magic_size = c::boot_magic_sz();

           if trailer_size % sector_size > 0 && trailer_size % sector_size < magic_size {
               required_padding += magic_size - trailer_size % sector_size;
           }
        }
    };
...
fn compute_largest_image_size(dev: &dyn Flash, areadesc: &AreaDesc, slots: &[SlotInfo],
    ...
    let padding = required_slot_padding(dev, trailer);
