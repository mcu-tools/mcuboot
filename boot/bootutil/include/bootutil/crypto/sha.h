...
static inline void bootutil_sha_init(bootutil_sha_context *ctx)
{
    ...
    ctx->error_code = mbedtls_sha512_starts(&ctx->mbedtls_ctx, 0);
#else
    mbedtls_sha256_init(&ctx->mbedtls_ctx);
    ctx->error_code = mbedtls_sha256_starts(&ctx->mbedtls_ctx, 0);
#endif
}
...
#ifdef MCUBOOT_SHA512
    ctx->error_code = mbedtls_sha512_update(&ctx->mbedtls_ctx, data, data_len);
#else
    ctx->error_code = mbedtls_sha256_update(&ctx->mbedtls_ctx, data, data_len);
#endif
}
...
#ifdef MCUBOOT_SHA512
    return mbedtls_sha512_finish(&ctx->mbedtls_ctx, output);
#else
    return mbedtls_sha256_finish(&ctx->mbedtls_ctx, output);
#endif
}
