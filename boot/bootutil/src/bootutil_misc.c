...
int boot_trailer_scramble_params(const struct flash_area *fa, size_t *off, size_t *size)
{
    int ret = 0;
    const size_t write_block = flash_area_align(fa);

    if (device_requires_erase(fa)) {
        /* For device requiring arese align to erase unit */
        struct flash_sector sector;

        ret = flash_area_get_sector(fa, flash_area_get_size(fa) - boot_trailer_sz(write_block),
                                    &sector);
        if (ret < 0) {
            return ret;
        }
        ...
        *size = flash_sector_get_size(&sector);
        ...
        /* For device not requirng erase align to write block */
        *off = ALIGN_DOWN(boot_trailer_sz(write_block), write_block);
        *size = ALIGN_UP(boot_trailer_sz(write_block), write_block);
    }
