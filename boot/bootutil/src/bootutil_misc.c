 ...
 *
 * NOTE: the table order is very important. The settings in secondary slot
 * always are priority to primary slot and should be located earlier in the
 * table.
 *
...
boot_read_swap_size(uint32_t *swap_size)
{
     ...
     * for a valid magic, first on primary slot, then on scratch. Both "slots"
    ...
    if (memcmp(magic, boot_img_magic, BOOT_MAGIC_SZ) != 0) {
        /*
         * If primary slot's magic is not valid, try scratch...
         */
...
/**
 * Marks the image in secondary slot as pending.  On the next reboot, the system
 * will perform a one-time boot of the secondary slot image.
 *
...
/**
 * Marks the image in primary slot as confirmed.  The system will continue
 * booting into the image in primary slot until told to boot from a different
 * slot.
 *
 * @return                  0 on success; nonzero on failure.
 */
