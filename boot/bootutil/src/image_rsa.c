...
/* Extract a member of the mbedtls_rsa_context structure. */
#if MBEDTLS_VERSION_NUMBER >= 0x03000000
#define MBEDTLS_RSA_CONTEXT_MEMBER(X) MBEDTLS_PRIVATE(X)
#else
#define MBEDTLS_RSA_CONTEXT_MEMBER(X) X
#endif
...
bootutil_parse_rsakey(mbedtls_rsa_context *ctx, uint8_t **p, uint8_t *end)
{
    ...
    if ((rc = mbedtls_asn1_get_mpi(p, end, &ctx->MBEDTLS_RSA_CONTEXT_MEMBER(N))) != 0 ||
        (rc = mbedtls_asn1_get_mpi(p, end, &ctx->MBEDTLS_RSA_CONTEXT_MEMBER(E))) != 0) {
        return -3;
    }

    ctx->MBEDTLS_RSA_CONTEXT_MEMBER(len) = mbedtls_mpi_size(&ctx->MBEDTLS_RSA_CONTEXT_MEMBER(N));
    ...
    rc = mbedtls_rsa_import(ctx, &ctx->MBEDTLS_RSA_CONTEXT_MEMBER(N), NULL,
                            NULL, NULL, &ctx->MBEDTLS_RSA_CONTEXT_MEMBER(E));
    if (rc != 0) {
        return -5;
    }
#endif
  ...
  uint8_t *sig)
{
    ...
    if (ctx->MBEDTLS_RSA_CONTEXT_MEMBER(len) != PSS_EMLEN ||
  ...
  uint8_t key_id)
{
    ...
    if (rc || slen != ctx.MBEDTLS_RSA_CONTEXT_MEMBER(len)) {
