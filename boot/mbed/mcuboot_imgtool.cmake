...
set(MCUBOOT_SIGNING_KEY "" CACHE STRING "Path to key file (.pem) used to sign firmware updates for your device.  This file must be kept safe!")
...
#
function(_mcuboot_generate_image TARGET IMAGE_TYPE IMAGE_BASENAME)
  # Figure out file name
  set(IMAGE_BASE_PATH ${CMAKE_CURRENT_BINARY_DIR}/${IMAGE_BASENAME})
  ...
  if(${IMAGE_TYPE} STREQUAL "initial")
    set(IMGTOOL_EXTRA_ARGS
      # this should only be used for binaries you plan on initially flashing to your target at the factory.
      # It pads the resulting binary to the slot size and adds initialized trailer TLVs. This is not needed
      # for update binaries.
      --pad)
  else()
    set(IMGTOOL_EXTRA_ARGS "")
  ...
  add_custom_command(
    ...
    COMMAND
      ...
      # this specifies the file containing the keys used to sign/verify the application
      --key ${MCUBOOT_SIGNING_KEY_ABSPATH}

      # this lets mcuboot know the intrinsic alignment of the flash (32-bits = 4 byte alignment)
      --align 4 # TODO does this need to be set on a per-target basis?

      # this sets the version number of the application
      --version ${PROJECT_VERSION}

      # this must be the same as the value specified in mcuboot.header-size configuration
      --header-size ${HEADER_SIZE_HEX}

      # this tells imgtool to insert the entire header, including any necessary padding bytes.
      --pad-header

      # this specifies the maximum size of the application ("slot size"). It must be the same as the configured mcuboot.slot-size!
      --slot-size ${SLOT_SIZE_HEX}

      ${IMGTOOL_EXTRA_ARGS}

      ${TARGET_HEX_FILE} ${IMAGE_BASE_PATH}.hex

    COMMAND ${CMAKE_COMMAND} -E echo "-- built: ${IMAGE_BASE_PATH}.hex"

    # Also generate bin file
    COMMAND
      ${objcopy} -I ihex -O binary ${IMAGE_BASE_PATH}.hex ${IMAGE_BASE_PATH}.bin

    COMMAND ${CMAKE_COMMAND} -E echo "-- built: ${IMAGE_BASE_PATH}.bin"
...
function(mcuboot_generate_initial_image TARGET) # optional 2nd arg: initial image base filename
  # Figure out file name
  if("${ARGN}" STREQUAL "")
    set(INITIAL_IMAGE_BASENAME $<TARGET_FILE_BASE_NAME:${TARGET}>-initial-image)
  else()
    set(INITIAL_IMAGE_BASE_PATH ${ARGN})
  endif()

  _mcuboot_generate_image(${TARGET} initial ${INITIAL_IMAGE_BASENAME})
endfunction(mcuboot_generate_initial_image)

#
# Generate an update image hex file for the given executable target.
# This initial image is what should be flashed to the secondary block device and passed to
...
function(mcuboot_generate_update_image TARGET) # optional 2nd arg: update image base filename
  # Figure out file name
  if("${ARGN}" STREQUAL "")
    set(UPDATE_IMAGE_BASENAME $<TARGET_FILE_BASE_NAME:${TARGET}>-update-image)
  else()
    set(UPDATE_IMAGE_BASENAME ${ARGN})
  endif()

  _mcuboot_generate_image(${TARGET} update ${UPDATE_IMAGE_BASENAME})
