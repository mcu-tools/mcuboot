...
def gen_rsa3072(keyfile, passwd):
    keys.RSA.generate(key_size=3072).export_private(path=keyfile, passwd=passwd)
...
@click.option('-p', '--password', is_flag=True, help='Prompt for password to protect key')
@click.option(
    '-t',
    '--type',
    metavar='type',
...
@click.option(
    ...
    help='This option is deprecated. Please use the `--encoding` option. Valid langs: {}'.format(
        ', '.join(valid_langs)
    ),
)
@click.option(
...
@click.option(
    ...
    help='Valid encodings: {}. Default value is {}.'.format(
        ', '.join(valid_hash_encodings), valid_hash_encodings[0]
    ),
)
@click.option('-k', '--key', metavar='filename', required=True)
@click.option(
...
@click.option(
    '-s', '--silent', default=False, is_flag=True, help='Do not print image information to output'
)
@click.command(help='Print header, TLV area and trailer information of a signed image')
...
def validate_header_size(ctx, param, value):
    ...
    if value < min_hdr_size:
        raise click.BadParameter(f"Minimum value for -H/--header-size is {min_hdr_size}")
    return value
...
def get_dependencies(ctx, param, value):
    if value is not None:
        ...
        if len(images) == 0:
            raise click.BadParameter(f"Image dependency format is invalid: {value}")
...
@click.option('--non-bootable', default=False, is_flag=True, help='Mark the image as non-bootable.')
@click.option(
    '--custom-tlv',
    required=False,
    nargs=2,
...
@click.option(
    ...
    help='When encrypting the image using AES, select a 128 bit or 256 bit key len.',
)
@click.option(
    '--compression',
    default='disabled',
...
@click.option(
    ...
    default='little',
    help="Select little or big endian",
)
@click.option(
    '--overwrite-only',
    default=False,
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help='When padding the image, mark it as confirmed (implies --pad)',
)
@click.option(
    '--test',
    default=False,
    is_flag=True,
    help='When padding the image, mark it for a test swap (implies --pad)',
)
@click.option(
    '--pad',
    default=False,
...
@click.option(
    ...
    type=BasedIntParamType(),
    required=True,
    help='Size of the slot. If the slots have different sizes, use the size of the secondary slot.',
)
@click.option(
    '--pad-header',
    default=False,
...
@click.option(
    '-H', '--header-size', callback=validate_header_size, type=BasedIntParamType(), required=True
)
@click.option(
    '--pad-sig',
    default=False,
    is_flag=True,
    help='Add 0-2 bytes of padding to ECDSA signature (for mcuboot <1.5)',
)
@click.option(
    '-d',
    '--dependencies',
    callback=get_dependencies,
    required=False,
    help='''Add dependence on another image, format:
              "(<image_ID>,<image_version>), ... "''',
)
@click.option(
    '-s',
    '--security-counter',
    callback=validate_security_counter,
...
@click.option(
    '--public-key-format',
    type=click.Choice(['hash', 'full']),
    default='hash',
    help='In what format to add the public key to the image manifest: full key or hash of the key.',
)
@click.option('-k', '--key', metavar='filename')
@click.option(
    '--fix-sig',
...
@click.option('--fix-sig-pubkey', metavar='filename', help='public key relevant to fixed signature')
@click.option(
    '--pure',
    'is_pure',
    is_flag=True,
...
@click.option(
    '--hmac-sha',
    'hmac_sha',
    type=click.Choice(valid_hmac_sha),
    default='auto',
    help='sha algorithm used in HKDF/HMAC in ECIES key exchange TLV',
)
@click.option(
    '--vector-to-sign',
    type=click.Choice(['payload', 'digest']),
    help='send to OUTFILE the payload or payload'
    's digest instead '
    'of complied image. These data can be used for external image '
    'signing',
)
@click.command(
    help='''Create a signed or unsigned image\n
...
def sign(
    ...
    if (
        ...
        and key
        and (
            (isinstance(key, keys.ECDSA256P1) and not isinstance(enckey, keys.ECDSA256P1Public))
            or (isinstance(key, keys.ECDSA384P1) and not isinstance(enckey, keys.ECDSA384P1Public))
            or (isinstance(key, keys.RSA) and not isinstance(enckey, keys.RSAPublic))
        )
    ):
        # FIXME
    ...
    for tlv in custom_tlv:
        ...
        if tag in image.TLV_VALUES.values():
            raise click.UsageError(f'Custom TLV {hex(tag)} conflicts with predefined TLV.')
    ...
    if raw_signature is not None:
        if fix_sig_pubkey is None:
            raise click.UsageError('public key of the fixed signature is not specified')

        pub_key = load_key(fix_sig_pubkey)

        baked_signature = {'value': raw_signature}
    ...
    if compression in ["lzma2", "lzma2armthumb"]:
        ...
        compression_filters = [
            {
                ...
                "preset": comp_default_preset,
                "dict_size": comp_default_dictsize,
                "lp": comp_default_lp,
                "lc": comp_default_lc,
            }
        ]
        ...
        compressed_data = lzma.compress(
            img.get_infile_data()[infile_offset:],
        ...
        compression_tlvs["DECOMP_SIZE"] = struct.pack(img.get_struct_endian() + 'L', img.image_size)
        ...
        if (compressed_size + compression_tlvs_size) < uncompressed_size:
            compression_header = create_lzma2_header(
                dictsize=comp_default_dictsize,
                pb=comp_default_pb,
                lc=comp_default_lc,
                lp=comp_default_lp,
            )
...
@click.command(cls=AliasesGroup, context_settings=dict(help_option_names=['-h', '--help']))
def imgtool():
    pass
