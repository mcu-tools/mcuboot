...
@click.option('--delta-base', metavar='filename',
              ...
              help='Create a robust signed reversible delta image against an '
                   'already-signed base image instead of a full update image.')
@click.option('--delta-block-size', default='16', type=BasedIntParamType(),
              help='Granularity, in bytes, used when comparing base and '
                   ...
                   'alignment, or an erase-block size that covers primary '
                   'records and reserves the primary and secondary trailer '
                   'blocks.')
...
def sign(key, public_key_format, align, version, pad_sig, header_size,
    ...
    if delta_base is not None:
        delta_block_size = int(delta_block_size)
        ...
        target_img.check_trailer(delta_block_size)
        ...
        if delta_payload.write_size + delta_block_size > slot_size:
            raise click.UsageError(
                f"Delta write span (0x{delta_payload.write_size:x}) + "
                f"reserved trailer area (0x{delta_block_size:x}) exceeds "
                f"requested size 0x{slot_size:x}")

        delta_tlvs = {
            "DELTA_BASE_SHA": delta_payload.base_hash,
            "OUTPUT_SHA": delta_payload.target_hash,
        }
        ...
        img.check_trailer(delta_block_size)
        ...
        compression_tlvs["OUTPUT_SHA"] = img.image_hash
        compression_tlvs_size = len(compression_tlvs["DECOMP_SIZE"])
        compression_tlvs_size += len(compression_tlvs["OUTPUT_SHA"])
