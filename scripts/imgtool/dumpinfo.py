...
def _human_format_tlv_records(tlv_list):
    ...
    for tlv in tlv_list:
        ...
        if tlv[tlv_type] in TLV_TYPES:
            lines.append(" " * indent + f" {tlv_type}: {TLV_TYPES[tlv[tlv_type]]} ({hex(tlv[tlv_type])})")
        else:
            lines.append(" " * indent + " {}: {} ({})".format(
                tlv_type, "UNKNOWN", hex(tlv[tlv_type])))
        lines.append(" " * indent + f" {tlv_length}:  {hex(tlv[tlv_length])}")

        data_line = " " * indent + f" {tlv_data}: "
...
def _write_format_human(imginfo, imgfile, outfile):
...
def dump_imginfo(imgfile, outfile=None, output_format=None, silent=False):
    ...
    try:
        ...
        if output_format == 'human':
            _write_format_human(imginfo, imgfile, out)
