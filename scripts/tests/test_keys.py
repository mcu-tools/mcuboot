...
@pytest.mark.parametrize(
    "bad_suffix",
    ["-dev", "foo/bar", "a b", "@2", "key+", " ", "_2-x"],
)
def test_getpub_name_suffix_invalid_chars_rejected(
    bad_suffix: str, tmp_path_persistent: Path,
) -> None:
    """Suffixes that would produce invalid C/Rust identifiers are rejected."""
    runner = CliRunner()
    gen_key = tmp_name(tmp_path_persistent, "ed25519", GEN_KEY_EXT)
    ...
    result = runner.invoke(
        ...
        [
            "getpub", "--key", str(gen_key),
            "--encoding", "lang-c",
            "--name-suffix", bad_suffix,
        ],
    )
    assert result.exit_code != 0
    assert "A-Za-z0-9_" in result.output


@pytest.mark.parametrize(
    "bad_suffix",
    ["-dev", "foo/bar", "a b", "@2", "key+", " "],
)
def test_getpubhash_name_suffix_invalid_chars_rejected(
    bad_suffix: str, tmp_path_persistent: Path,
) -> None:
    """getpubhash rejects suffixes with non-identifier chars too."""
    runner = CliRunner()
    gen_key = tmp_name(tmp_path_persistent, "ed25519", GEN_KEY_EXT)
