...
1. Check if the storage has been erased. If it hasn't, nothing will happen.
2. Generate a secure random number.
3. Create ECC key pair (SECP256R1).
4. Convert the private key to DER format (PKCS#8).
5. Store the private key into the flash partition.
6. Convert the public key to PEM format.
