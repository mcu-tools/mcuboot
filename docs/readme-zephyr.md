...
if primary slot should be validated on every boot.
...
need to be programmed at the offset of primary slot for this particular target.
Depending on the platform and flash tool you might need to manually specify a
flash offset corresponding to the primary slot starting address. This is usually
...
MCUboot.
These images can also be marked for upgrade, and loaded into secondary slot,
at which point the bootloader should perform an upgrade.  It is up to
the image to mark primary slot as "image ok" before the next reboot,
